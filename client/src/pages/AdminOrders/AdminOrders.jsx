// src/pages/AdminOrders/AdminOrders.jsx
...
function AdminOrders() {
  ...
  const handleStatusChange = (orderId, newStatus) => {
    ...
    const updatedOrders = orders.map((order) => {
      ...
      const lastHistoryStatus =
        existingHistory[existingHistory.length - 1]?.status;

      const updatedHistory =
        lastHistoryStatus === newStatus
          ? existingHistory
          : [
              ...existingHistory,
              {
  ...
  const pendingOrders = orders.filter(
    (order) =>
      Number(order.statusStep || 1) > 0 && Number(order.statusStep || 1) < 4,
  ).length;
